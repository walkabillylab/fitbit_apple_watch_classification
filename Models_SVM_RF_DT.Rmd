...
ggplot(aggregated_data, aes(x = DateTime, y = Fitbit.Heart)) + 
  geom_point() + 
  facet_wrap(~ id)

ggplot(aggregated_data, aes(Fitbit.Steps)) + 
  geom_histogram()

ggplot(aggregated_data, aes(Applewatch.Steps_LE)) + 
  geom_histogram()
...
##### Fitbit with non-interpolated data, removing Nas with rfImpute - Random Forest and SVM
```{r}
x_columns_FB <- c(
  "age",
  "gender",
...
summary_data_before_removing_na <- aggregated_data %>% 
  summarise(
    ...
    n_Fitbit.Steps_LE = length(which(!is.na(Fitbit.Steps_LE))),
    n_Fitbit.Heart_LE = length(which(!is.na(Fitbit.Heart_LE))),
    n_Fitbit.Calories_LE = length(which(!is.na(Fitbit.Calories_LE))),
    n_FFitbit.Distance_LE = length(which(!is.na(Fitbit.Distance_LE))),
    n_EntropyFitbitHeartPerDay_LE = length(which(!is.na(EntropyFitbitHeartPerDay_LE))),
    n_EntropyFitbitStepsPerDay_LE = length(which(!is.na(EntropyFitbitStepsPerDay_LE))),
    n_RestingFitbitHeartrate_LE = length(which(!is.na(RestingFitbitHeartrate_LE))),
    n_CorrelationFitbitHeartrateSteps_LE = length(which(!is.na(CorrelationFitbitHeartrateSteps_LE))),
    n_NormalizedFitbitHeartrate_LE = length(which(!is.na(NormalizedFitbitHeartrate_LE))),
    n_FitbitIntensity_LE = length(which(!is.na(FitbitIntensity_LE))),
    n_SDNormalizedFitbitHR_LE = length(which(!is.na(SDNormalizedFitbitHR_LE))),
    n_FitbitStepsXDistance_LE = length(which(!is.na(FitbitStepsXDistance_LE)))
  )
...
x <- rfImpute(activity_trimmed ~ ., x)
# remove the activity_trimmed column
x <- x[-1]




summary_data_after_removing_na <- x %>% 
  summarise(
...
##### Applewatch with non-interpolated data, imputed - Random Forest and SVM
```{r}
x_columns_AW <- c(
  "age",
  "gender",
...
| Description  | Value |               |
| ------------- | ------------- |---------------|
| Correctly Classified Instances  | 2012  | 76.531  % |
| Incorrectly Classified Instances  | 617  | 23.469  % |
|Kappa statistic | 0.723 |   |
|Mean absolute error | 0.104 |   |
|Root mean squared error | 0.2182 |  |
|Relative absolute error | 42.8357 % |  |
|Root relative squared error | 62.6229 % |  |
|Total Number of Instances | 2629 |  |     

##### Detailed Accuracy By Class
| X | TP Rate | FP Rate | Precision | Recall | F-Measure | MCC | ROC Area | PRC Area | Class |
|--|---------|---------|-----------|--------|-----------|-----|----------|----------|-------|
|  |0.822    |0.061    |0.770      |0.822   |0.795      |0.742|    0.961 |    0.890 | Lying |
|  |0.862    |0.042    |0.787      |0.862   |0.823      |0.790|    0.975 |    0.899 | Running 5 METs|
|  |0.254    |0.052    |0.373      |0.254   |0.302      |0.240|    0.739 |    0.352 | transit|
|  |0.695    |0.045    |0.699      |0.695   |0.697      |0.652|    0.938 |    0.746 | Sitting|
|  |0.904    |0.025    |0.888      |0.904   |0.896      |0.873|    0.986 |    0.961 | Running 7 METs|
|  |0.815    |0.029    |0.798      |0.815   |0.806      |0.779|    0.982 |    0.887 | Running 3 METs|
|  |0.834    |0.022    |0.823      |0.834   |0.828      |0.807|    0.974 |    0.901 | Self Pace walk|
|Weighted Avg.|    0.765 |    0.040 |    0.751 |      0.765 |    0.756 |      0.719 |    0.944 |     0.828 | |

##### Confusion Matrix
| Lying | Running 5 METs | transit | Sitting | Running 7 METs | Running 3 METs | Self Pace walk |
|-------|----------------|---------|---------|----------------|----------------|----------------|
| 433|   5 |  32 |  19 |   1 |  18 |  19 |
| 3 | 343 |  14 |  12 |  13 |  11 |   2 |
| 66 |  28 |  72 |  51 |  30 |  24 |  13 |
| 28  | 24 |  31 | 237 |   9 |   7 |   5 |
| 1 |  19 |  18 |   6 | 426 |   1 |   0 |
| 18  |  9 |  16 |   3 |   0 | 260 |  13 |
| 13   | 8 |  10 |  11 |   1 |   5 | 241 |
...
| Description  | Value |               |
| ------------- | ------------- |---------------|
| Correctly Classified Instances  | 293  | 76.1039 % |
| Incorrectly Classified Instances  | 92  | 23.8961 % |
|Kappa statistic | 0.7085 |   |
|Mean absolute error | 0.1057 |   |
|Root mean squared error | 0.2215 |  |
|Relative absolute error | 44.8496 % |  |
|Root relative squared error | 64.5576 % |  |
|Total Number of Instances | 385 |  |     

##### Detailed Accuracy By Class
| X | TP Rate | FP Rate | Precision | Recall | F-Measure | MCC | ROC Area | PRC Area | Class |
|--|---------|---------|-----------|--------|-----------|-----|----------|----------|-------|
|  |0.835    |0.061    |0.809      |0.835   |0.822      |0.765|    0.953 |    0.909 | Lying |
|  |0.587    |0.065    |0.551      |0.587   |0.568      |0.508|    0.911 |    0.535 | Running 5 METs|
|  |0.171    |0.037    |0.316      |0.171   |0.222      |0.178|    0.737 |    0.263 | transit|
|  |0.861    |0.017    |0.838      |0.861   |0.849      |0.834|    0.986 |    0.912 | Sitting|
|  |0.690    |0.028    |0.667      |0.690   |0.678      |0.651|    0.929 |    0.785 | Running 7 METs|
|  |0.729    |0.042    |0.714      |0.729   |0.722      |0.682|    0.939 |    0.784 | Running 3 METs|
|  |0.980    |0.032    |0.916      |0.980   |0.947      |0.928|    0.994 |    0.987 | Self Pace walk|
|Weighted Avg.|    0.761 |    0.043 |    0.741 |      0.761 |    0.749 |      0.711 |    0.939 |     0.801 | |
